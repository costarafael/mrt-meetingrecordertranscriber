...
# --- DETECÇÃO DE CAMINHO ABSOLUTO PARA PIPELINE_SWIFT ---
SCRIPT_DIR=$( cd -- "$( dirname -- "${BASH_SOURCE[0]}" )" &> /dev/null && pwd )
# Assumindo que estamos em pipeline_swift/src
PIPELINE_ROOT="$SCRIPT_DIR/.."
MODELS_DIR="$PIPELINE_ROOT/models"
LIBS_DIR="$PIPELINE_ROOT/build/build-swift-macos"
# --- FIM DA DETECÇÃO DE CAMINHO ---

# Verifica se foi passado o modelo como parâmetro
if [ "$1" == "small" ]; then
    WHISPER_MODEL="small"
elif [ "$1" == "base" ]; then
    WHISPER_MODEL="base"
else
    echo "Uso: $0 [small|base]"
    echo "Modelos disponíveis: small (mais preciso), base (mais rápido)"
    echo "Usando modelo padrão: small"
    WHISPER_MODEL="small"
fi

# Define nomes de arquivos com base no modelo
OUTPUT_NAME="pipeline-optimized-${WHISPER_MODEL}"
SWIFT_FILE="pipeline-optimized-${WHISPER_MODEL}.swift"

# Usar << EOF (sem aspas) e escapar os '$' do Swift com '\'
cat > "$SWIFT_FILE" << EOF
import Foundation
import AVFoundation
import Accelerate
...
struct FinalPipelineRunner {
    static func main() {
        ...
        let config = PipelineConfig(
            ...
            modelsBasePath: "${MODELS_DIR}",
            whisperModelSize: "${WHISPER_MODEL}"
        )
...
# Compilação e execução do código Swift
echo "⚙️  Compilando o pipeline otimizado para Apple Silicon..."
swiftc \
  -O \
  -lc++ -lstdc++ \
  -I ${LIBS_DIR}/install/include \
  -import-objc-header ./SherpaOnnx-Bridging-Header.h \
  ./"$SWIFT_FILE" ./SherpaOnnx.swift \
  ${LIBS_DIR}/install/lib/libsherpa-onnx.a \
  ${LIBS_DIR}/install/lib/libonnxruntime.a \
  ${LIBS_DIR}/install/lib/libsherpa-onnx-c-api.a \
  ${LIBS_DIR}/install/lib/libsherpa-onnx-core.a \
  ${LIBS_DIR}/install/lib/libkaldi-native-fbank-core.a \
  -framework Foundation -framework AVFoundation -framework Accelerate \
  -o "$OUTPUT_NAME"
...
if [ $? -eq 0 ]; then
  ...
  echo "❌ Erro na compilação do Swift."
  exit 1
fi
...
class SystemAudioCaptureService: SystemAudioCaptureProtocol {
    ...
    private func setupSystemAudioPipeline(configuration: AudioConfiguration) async throws {
        ...
        // 🔧 NOVA ESTRATÉGIA: Detectar melhor opção baseada na versão do macOS
        let osVersion = ProcessInfo.processInfo.operatingSystemVersion
        let shouldUseCoreAudioTaps = osVersion.majorVersion >= 14 && osVersion.minorVersion >= 2
        
        if shouldUseCoreAudioTaps {
            logger.info("🔧 Sistema detectou macOS 14.2+: Preferindo Core Audio Taps para estabilidade")
            // TODO: Implementar Core Audio Taps quando estiver pronto
            logger.warning("⚠️ Core Audio Taps ainda não implementado, usando ScreenCaptureKit como fallback")
        }
        
        // Criar pipeline ScreenCaptureKit (mantido como principal por enquanto)
        ...
        // 🔧 NOVO: Configurar callback para detectar falhas
        screenCaptureKitPipeline?.onStreamFailure = { [weak self] error in
            Task {
                await self?.handleScreenCaptureKitFailure(error)
            }
        }
        ...
        logger.info("🔧 Pipeline de áudio do sistema configurado com detecção de falhas")
    }
    
    // 🔧 NOVO: Lidar com falhas do ScreenCaptureKit
    private func handleScreenCaptureKitFailure(_ error: Error) async {
        logger.error("❌ ScreenCaptureKit falhou: \(error)")
        logger.warning("🔄 Implementando estratégia de fallback...")
        
        // Por enquanto, apenas logar a falha
        // TODO: Implementar fallback para Core Audio Taps ou outra estratégia
        logger.warning("⚠️ Fallback automático ainda não implementado")
        logger.warning("   • Considere usar uma versão mais recente do macOS")
        logger.warning("   • Ou aguarde implementação de Core Audio Taps")
    }

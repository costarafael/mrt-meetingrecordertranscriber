...
struct RecordingView: View {
    ...
    var body: some View {
        VStack(spacing: 20) {
            ...
            VStack(spacing: 12) {
                ...
                if audioService.systemAudioAvailable {
                    VStack(spacing: 4) {
                        ...
                        // Mostrar estratégia de captura ativa
                        if audioService.systemAudioEnabled {
                            HStack {
                                Image(systemName: audioService.activeAudioCaptureStrategy == .coreAudioTaps ? "waveform.badge.mic" : "display")
                                    .foregroundColor(audioService.activeAudioCaptureStrategy == .coreAudioTaps ? .orange : .blue)
                                    .font(.caption2)
                                
                                Text(audioService.activeAudioCaptureStrategy == .coreAudioTaps ? 
            ...
            // Tempo de gravação
            Text(audioService.currentDuration.adaptiveFormat)
